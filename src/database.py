...
class Database:
    ...
    def get_matches(self):
        ...
        cursor.execute('SELECT m.match_date, p.name, r1.name, r2.name, result '
                       'FROM matches AS m '
                       'LEFT JOIN players AS p ON m.partner = p.id '
        ...
        result = []
        for row in rows:
            result.append({"date": row[0], "partner": row[1], "rival1": row[2], "rival2": row[3], "result": row[4]})
